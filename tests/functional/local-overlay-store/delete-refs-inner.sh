...
expectStderr 1 nix-store --delete "$input1" | grepQuiet "Cannot delete some of the given paths because they are still alive"
expectStderr 1 nix-store --delete "$input2" | grepQuiet "Cannot delete some of the given paths because they are still alive"
expectStderr 1 nix-store --delete "$input3" | grepQuiet "Cannot delete some of the given paths because they are still alive"

# These same paths are referenced in the lower layer (by the seed 1
# build done in `initLowerStore`).
expectStderr 1 nix-store --store "$storeA" --delete "$input2" | grepQuiet "Cannot delete some of the given paths because they are still alive"
expectStderr 1 nix-store --store "$storeA" --delete "$input3" | grepQuiet "Cannot delete some of the given paths because they are still alive"
