...
TEST_F(ValuePrintingTests, depthAttrs)
{
    ...
    test(
        vNested,
        "{ nested = { empty = { }; nested = { ... }; one = 1; two = 2; }; one = 1; two = 2; }",
        PrintOptions{.maxDepth = 2});
    test(
        vNested,
        "{ nested = { empty = { }; nested = { zero = 0; }; one = 1; two = 2; }; one = 1; two = 2; }",
        PrintOptions{.maxDepth = 3});
    test(
        vNested,
        "{ nested = { empty = { }; nested = { zero = 0; }; one = 1; two = 2; }; one = 1; two = 2; }",
        PrintOptions{.maxDepth = 4});
}
...
TEST_F(ValuePrintingTests, listRepeated)
{
    ...
    test(vList, "[ { x = 0; } { x = 0; } ]", PrintOptions{.trackRepeated = false});
}
