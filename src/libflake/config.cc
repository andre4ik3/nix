...
void ConfigFile::apply(const Settings & flakeSettings)
{
    ...
    for (auto & [name, value] : settings) {
        ...
        if (!trusted) {
            switch (flakeSettings.acceptFlakeConfig.get()) {
            ...
            case AcceptFlakeConfig::Ask: {
                ...
                if (auto saved = tlname ? get(*tlname, valueS) : nullptr) {
                    ...
                    if (std::tolower(
                            logger
                                ->ask(
                                    fmt("Do you want to allow configuration setting '%s' to be set to '" ANSI_RED
                                        "%s" ANSI_NORMAL
                                        "' (y/N)? This may allow the flake to gain root, see the nix.conf manual page.",
                                        name,
                                        valueS))
                                .value_or('n'))
                        == 'y') {
                        ...
                        warn(
                            "you can set '%s' to '%s' to automatically reject configuration options supplied by flakes",
                            "accept-flake-config",
                            "false");
                    }
        ...
        if (trusted) {
            ...
            warn(
                "ignoring untrusted flake configuration setting '%s', pass '%s' to trust it (may allow the flake to gain root, see the nix.conf manual page)",
                name,
                "--accept-flake-config");
        }
    }
